// Gmail OAuth Service - Fixed version with dynamic Supabase loading
...
const getRedirectUri = () => {
  ...
  if (currentOrigin.includes('localhost') || currentOrigin.includes('192.168') || currentOrigin.includes('127.0.0.1')) {
    return 'http://localhost:8081/branding';
  }
  
  // For production
  return `${currentOrigin}/branding`;
};
...
const getSupabaseClient = async () => {
  ...
  try {
    ...
    try {
      ...
      console.warn('Alias import failed, trying relative:', aliasError.message);
      try {
        supabaseModule = await import('../integrations/supabase/client.js');
      } catch (relativeError) {
        console.warn('Relative import failed, trying absolute:', relativeError.message);
        supabaseModule = await import('/src/integrations/supabase/client.js');
      }
    }
    ...
    if (supabaseModule && supabaseModule.supabase) {
      ...
      throw new Error('Supabase client not found in module');
    }
  } catch (error) {
...
export const getGmailProfile = async (accessToken) => {
  try {
    // Try Google userinfo API first (more reliable)
    ...
    console.log('Userinfo API failed, trying Gmail profile API...');
